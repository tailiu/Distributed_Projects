...
var createPeer = function(host, port) {

  helper.log("createPeer", "cli.js", "Enter");
            ...
            rpcClient.call(
              {
                ...
                "id"      : randstr.generate(),
                  ...
                  if (err) {
                    helper.log("createPeer > rpcClient", "cli.js", "Error here");
                  } else{
                    helper.log("createPeer > rpcClient", "cli.js", "No Error");
                  }
                  helper.log("createPeer > rpcClient", "cli.js", JSON.stringify(res));
                  callback(err, response);
              }
            );
        }
    };
    helper.log("createPeer", "cli.js", "Return");
}

var createSender = function(port){

  helper.log("createSender", "cli.js", "Enter");

  var rpcClient = new rpc.Client({port: port, host: HOST});
  return function(rpcName, content, callback){
    rpcClient.call(
      {
        "jsonrpc" : 2.0,
        "method" : rpcName,
        ...
        callback = callback || function() {};
        if (err) {
          helper.log("createSender > rpcClient", "cli.js", "Error here");
        } else{
          helper.log("createSender > rpcClient", "cli.js", "No Error");
        }
        helper.log("createSender > rpcClient", "cli.js", JSON.stringify(res));
        callback(err, response);
      }
    );
  }
  helper.log("createSender", "cli.js", "Return");
}
    ...
    if (helper.greaterThan(proposal, promisedProposal)) {
        ...
        _.each(PEERS, function(peer) {
          ...
          rpcClient.call(
            ...
            function(req, res){
              helper.log("handleAccept > rpcClient", "cli.js", "Came back from learn RPC from "+peer.port, {req, res});
            }
          );
        });
        return {
      ...
      prompt.get(['cmd'], function(err, res){
        ...
        switch (res.cmd) {
          ...
          case "exit":
            process.exit();
          case "peers":
            helper.log("mainLoop", "cli.js", JSON.stringify(PEERS));
            break;
          default:{
