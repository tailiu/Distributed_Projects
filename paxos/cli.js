    ...
    if (helper.greaterThan(proposal, promisedProposal)) {
        ...
        _.each(PEERS, function(peer) {
          ...
          rpcClient.call(
            ...
            function(req, res){
              helper.log("handleAccept > rpcClient", "cli.js", "Received Learn Response", {"peer": peer, "req": req, "res": res});
              helper.log("handleAccept > rpcClient", "cli.js", "-- req", req);
              helper.log("handleAccept > rpcClient", "cli.js", "-- res", res);

              if(res){
                if(res.result){
                  if (res.result.name.indexOf("#join") > -1) {
                    helper.log("handleAccept > rpcClient", "cli.js", "Initiating connection", res.result.value);
...
var handleLearn = function(name, instance, value, peer) {
    ...
    if (FULLY_LEARNT_VALUES[name].hasOwnProperty(instance)) {
        ...
        helper.log("handleLearn", "cli.js", "Return with:", what_did_i_learn);
...
var initiateAccept = function(name, instance, proposal, value, originalValue, finalResponse) {
          ...
          rpcClient.call(
            {
              ...
              "id"      : randstr.generate(),
                ...
                var received = null;
                if ((err && err.code === TIMEOUT_ERROR) || response.error) {
    ...
    async.parallel(acceptTasks, function(err, accepted) {
        ...
        _.each(accepted, function(response) {
            if (response === TIMEOUT_ERROR || response === DROPPED_ERROR || response.error) {
...
var reqConnRPC = function(req, callback){

  helper.log("reqConnRPC", "cli.js", "Remote Peer", req);

  if("#join" in FULLY_LEARNT_VALUES){
    helper.log("reqConnRPC", "cli.js", "Known Peers", FULLY_LEARNT_VALUES["#join"]);
    async.forEach(
      Object.entries(FULLY_LEARNT_VALUES["join"].values),
      function(node, callback) {
        var node = JSON.parse(node);
        if(node.port == req.node.port && req.node.host == node.host){
          helper.log("reqConnRPC", "cli.js", "Peer Found!", node);
          connect(req.node.port, req.node.host, PORT, HOST);
          callback(true);
        }
        callback(false);
      },
      function(found) {
        if(found) {
            ...
            console.log("Peer ain't found.");
            // newConnInit(req.node);
        }
      }
    );
  }else{
    helper.log("reqConnRPC", "cli.js", "Init new connection for", req);
    newConnInit(req);
  }
}
...
var getVal = function(name, instance) {
  ...
  if (PRETEND_DEAD) return TIMEOUT_ERROR;
...
var addNode = function(args, callback) {
  ...
  helper.log("addNode", "cli.js", JSON.stringify(args));
    ...
    helper.log("addNode", "cli.js", "All Peers", PEERS);
    helper.log("addNode", "cli.js", "Peer Added" ,PEERS[peerPort]);
...
var reqConnect = function(rport, rhost, port, host){

  helper.log("reqConnect", "cli.js", "Enter");
  helper.log("reqConnect", "cli.js", "Params - rport: {0}, rhost: {1}, port: {2}, host: {3}"
              .replace("{0}", rport).replace("{1}", rhost).replace("{2}", port).replace("{3}", host));
  ...
  rpcClient.call(
    {
      ...
      helper.log("reqConnect>rpcClient.call", "cli.js", JSON.stringify(res));
      if (err) {
        helper.log("reqConnect>rpcClient.call", "cli.js", "SOMETHING HAPPENED AT SERVER RPC!");
      }else{
        helper.log("reqConnect>rpcClient.call", "cli.js", "EXECUTED SUCCESFULLY!");
        // addNode(res.result, function(){});
      }
    }
  );
...
var newConnInit = function(req, callback){

  helper.log("newConnInit", "cli.js", "node:", req.node);
  var val = JSON.stringify({port:req.node.port, host:req.node.host});
  helper.log("newConnInit", "cli.js", "val:", val);
  putVal("#join", val);

}

var proceedWithJoin = function(node, peers){

  helper.log("proceedWithJoin", "cli.js", "Enter");
  helper.log("proceedWithJoin", "cli.js", "Params", {node: node, peers: peers});

  var rpcClient = new rpc.Client({port: node.port, host: node.host});

  rpcClient.call(
    {
      "jsonrpc" : 2.0,
      "method"  : "initJoinRPC",
      "params"  : {"node" : {port: PORT, host: HOST}, "peers" : PEERS},
      "id"      : randstr.generate(),
    }, function(err, res){
      helper.log("proceedWithJoin", "cli.js", JSON.stringify(res));
      if (err) {
        helper.log("proceedWithJoin", "cli.js", "SOMETHING HAPPENED AT SERVER RPC!");
      }else{
        helper.log("proceedWithJoin", "cli.js", "EXECUTED SUCCESFULLY!");
      }
    }
  );
}

var initJoinRPC = function(req, callback){

  helper.log("initJoinRPC", "cli.js", "Enter");
  helper.log("initJoinRPC", "cli.js", "Params", req);

  _.each(req.peers, function(peer){
    connect(peer.port, peer.host, PORT, HOST);
  });
}

var cleanDead = function(req, callback){

    helper.log("cleanDead", "cli.js", "Enter");
    ...
    helper.log("cleanDead", "cli.js", "Return");
}

var cleanDeadRPC = function(){

  helper.log("cleanDeadRPC", "cli.js", "Enter");

  _.each(PEERS, function(node){
    var rpcClient = new rpc.Client({port: node.port, host: node.host});
    ...
    rpcClient.call(
      {
        ...
        helper.log("cleanDeadRPC", "cli.js", JSON.stringify(res));
        if (err) {
          helper.log("cleanDeadRPC", "cli.js", "SOMETHING HAPPENED AT SERVER RPC!");
        }else{
          helper.log("cleanDeadRPC", "cli.js", "EXECUTED SUCCESFULLY!");
          process.exit();
        }
      }
    );
...
rpcServer.addMethod('reqConnRPC', newConnInit);
rpcServer.addMethod('initJoinRPC', initJoinRPC);
rpcServer.addMethod('cleanDead', cleanDead);
      ...
      prompt.get(['cmd'], function(err, res){
        ...
        switch (res.cmd) {
          ...
          case "peers":
            ...
            break;
          case "die":
            helper.log("mainLoop", "cli.js", "**Dead**");
            PRETEND_DEAD = true;
            cleanDeadRPC();
            // process.exit();
            break;
          case "alive":
            helper.log("mainLoop", "cli.js", "**Back On**");
            PRETEND_DEAD = false;
            break;
          case "values":
          ...
          default:{
            ...
            if (args.length == 2 && args[0] == "join") {
              if (args[1] == PORT) {
                ...
                reqConnect(args[1], HOST, PORT, HOST);
              }
